...
if (looks_like_number($content)) {
        ...
        $value =~ s/\R//g;
        $value=" value=".$value.";;;0";
        #print "looks line number \n";
}


my $re="(Status:)(\\s+)((?:([A-Z]|[a-z])+))(\\s+)(\\|)((?:([A-Z]|[a-z])+))(=)((([+-]?\\d*\\.\\d+)(?![-+0-9\\.]))|(((\\+|\\-|)\\d+)))(([A-Z]|[a-z]))(;)(\.*?)(;)(\.*?)(;)(\\d+)";
...
##parse response if passthrough

$value =~ s/.*\|//;

my $rawvalue = $value;
$rawvalue =~ s/=/;/g;
my @checks = split(/ /, $rawvalue);
my @errors;
#print "Dumper:\n";
#print Dumper(\@checks);
my $state;
...
foreach $check (@checks){
        my @currcheck = split(';',$check);
        #print Dumper(\@currcheck);
        my $r_label = @currcheck[0];
        my $r_value = @currcheck[1];
        $r_value =~ s/[^\d.-]+//g;
        my $r_warn = @currcheck[2];
        my $r_crit = @currcheck[3];
        my $r_min = @currcheck[4];
        my $r_max = @currcheck[5];
        if (($r_crit ne '') and ($r_warn ne '')) {
                if($r_value > $r_crit){
                        $state = "CRITICAL";
                        $statusmessage = "Status: Critical ($r_label=$r_value > $r_crit )|$value";
                        print $statusmessage."\n";
                        exit $ERRORS{$state};
                ...
                if($r_value > $r_warn){
                        $state = "WARNING";
                        $statusmessage = "Status: Warning ($r_label=$r_value > $r_warn )|$value";
...
if ($response->is_success) {
   # print "Status: OK |time=$delta" . "s;;;0".$value."\n";
